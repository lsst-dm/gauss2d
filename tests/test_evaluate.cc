...
TEST_CASE("Evaluator")
{
    ...
    for(size_t i = 0; i < n_comp; ++i)
    {
        ...
        data.emplace_back(std::make_shared<g2::ConvolvedGaussian>(
            std::make_shared<const g2::Gaussian>(
                std::make_shared<g2::Centroid>(std::make_shared<g2::CentroidValues>(std::move(x), std::move(y))),
            ...
            std::make_shared<const g2::Gaussian>(
                std::make_shared<g2::Centroid>(0, 0),
    ...
    auto eval_img = std::make_shared<Evaluator>(
        gaussians,
        nullptr,
        nullptr,
        nullptr,
    ...
    *image += err;
    ...
    auto eval_like = std::make_shared<Evaluator>(
        gaussians,
        nullptr,
        image,
        sigma_inv
    ...
    for(size_t idx_param = 0; idx_param < n_params; idx_param++) data_jacs.push_back(
        std::make_shared<Image>(n_rows, n_cols));
...
*/
    ...
    *image += -err;
    ...
    auto eval_img_new = std::make_shared<Evaluator>(
        gaussians,
        nullptr,
        nullptr,
        nullptr,
